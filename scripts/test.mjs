...
test('展示名・通称（別名）で引ける', () => {
  ...
  // 流通名で登録されている種を、標準和名でも引けること
  hit(frogs, 'ジュウジメドクアマガエル', 'アマゾンミルクフロッグ');
  hit(byGroup.get('gecko'), 'ツノミカドヤモリ', 'ガーゴイルゲッコー');
  hit(byGroup.get('lizard'), 'サバンナオオトカゲ', 'サバンナモニター');
  hit(byGroup.get('snake'), 'ミルクヘビ', 'ミルクスネーク');
});
